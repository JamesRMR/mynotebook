...
class PubSubClient:
    ...
    def pull(
        ...
        """Synchronously pull messages without acknowledging them.

        Unlike :meth:`subscribe`, this just leases and returns messages for viewing.
        Each returned message keeps its `ack_id`; pass it to :meth:`ack` or :meth:`nack`
        once you decide what to do with it. Anything left un-acked becomes available
        again after the subscription's ack deadline elapses.

        Args:
            subscription:   short subscription name (not the full resource path).
            max_messages:   max number of messages to lease in this call.
            timeout:        seconds to wait for messages before returning.

        Returns:
            list of :class:`ReceivedMessage`, each with `.ack_id` and `.message`.
        """
        response = self.subscriber.pull(
            request={
                "subscription": self._subscription_path(subscription),
    ...
    def ack(self, subscription: str, ack_ids: list[str]) -> None:
        """Acknowledge messages, removing them from the subscription."""
    ...
    def nack(self, subscription: str, ack_ids: list[str]) -> None:
        """Nack messages, making them available for redelivery immediately."""
